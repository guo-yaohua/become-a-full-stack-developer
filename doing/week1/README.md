...
Java 语言中的多态：同一个对象的行为，在不同的时刻或条件下表现出不同的效果。
...
多态成员的访问特征：父类引用指向子类对象的时候，去访问子类对象中的成员时：
- 成员变量：编译看左边，运行看左边。  
...
instanceof 运算符的作用是判断其左边对象是否为其右边类的实例，返回 boolean 类型的数据。可以用来判断继承中的子类的实例是否为父类的实现。  
