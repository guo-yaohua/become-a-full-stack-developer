...
Java 中有一种功能很强的循环结构，可以用来依次处理数组中的每个元素，而不必为指定下标值而分心。即 for each 循环。  
...
```  
for each 中的循环变量会变量会遍历数组中的每一个元素，而不需要下标值。
...
数组的概念（一组数据）：相同数据类型的数据元素的有序（物理地址有序非元素有序）集合。通过一个整型下标就可以访问数组中的每一个元素。  
- 存储多个数据元素
...
数组拷贝：在 Java 中，允许将一个数组拷贝给另一个数组变量。这时，两个变量将引用同一个数组。
